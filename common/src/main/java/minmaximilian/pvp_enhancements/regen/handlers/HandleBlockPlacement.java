...
public class HandleBlockPlacement {
    public static void handleBlockPlacement(LevelAccessor levelAccessor, @Nullable Entity entity, BlockState placedBlock, BlockPos blockPos) {
        if (entity instanceof Player) {
            Block block = placedBlock.getBlock();
            ChunkTracker chunkTracker = ChunkData.getChunkTracker(levelAccessor.dimensionType().effectsLocation(), new ChunkPos((blockPos)));
            ServerLevel level = levelAccessor.getServer().getLevel(ResourceKey.create(Registry.DIMENSION_REGISTRY, levelAccessor.dimensionType().effectsLocation()));
